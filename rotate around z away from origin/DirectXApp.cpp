...
{
	// This is for the rotation angle around the Z-axis
	_rotationAngle = (_rotationAngle + 1) % 360;

	// Calculate the new position of the object (assuming a constant distance of 2 units from the origin)
	float radius = 2.0f;
	float x = radius * cos(_rotationAngle * XM_PI / 180);
	float y = radius * sin(_rotationAngle * XM_PI / 180);

	_worldTransformation = Matrix::CreateTranslation(Vector3(x, y, 0)) * Matrix::CreateRotationZ(_rotationAngle * XM_PI / 180);
} 
